...
( function() {
	...
	/**
	 * 优化数据结构以兼容kindeditor的缩进处理
	 */
	function normalizeKE(html){
		var container = new CKEDITOR.dom.element('div');
			container.setHtml(html);
		
		handle(container);
		
		return kindEditorCompat(container.getHtml());//container.getHtml();//
		...
		function getChilds(ele){
			var nodeList = ele.getChildren(),
				childs = [];
			
			for(var i = 0;i < nodeList.count();i++){
				var child = nodeList.getItem(i);
				if(child.$.nodeType == CKEDITOR.NODE_TEXT && child.getLength == 0){
					continue;
				}
				childs.push(nodeList.getItem(i));
			}
		...
		function handle(ele){
			...
			//如果本身是blockquote节点,则要对子节点进行优化
			if(ele.getName() === 'blockquote'){
				
				//当所有子节点都处理完后，子节点可能会发生变化，重新获取子节点
				childs = getChilds(ele);
				len = childs.length;
				
				var temp = [];
				for(var i = 0;i < len;i++){
					var node = childs[i];
					if(node.$.nodeType == CKEDITOR.NODE_ELEMENT && node.getName() === 'blockquote'){
						temp.push(i);
					}
				}
				...
				while(temp.length > 0 && len > 1){
					...
					//插入curr之前的
					if(curr - prev > 1){
						var blockquote = new CKEDITOR.dom.element('blockquote');
						for(j = prev + 1;j < curr;j++){
							blockquote.append(childs[j]);
						}
						blockquote.insertBefore(ele);
					}
					
					//插入curr
					var blockquote = new CKEDITOR.dom.element('blockquote');
					blockquote.append(childs[curr]);
					blockquote.insertBefore(ele);
				}
				
				if(curr > -1 && curr < len - 1){
					var blockquote = new CKEDITOR.dom.element('blockquote');
					for(var i = curr + 1;i < len;i++){
						blockquote.append(childs[i]);
					}
					blockquote.insertBefore(ele);
				}
		...
		function kindEditorCompat(data){
			var reg = /(\s*<blockquote[^>]*>\s*)+(?:<p[^>]*>)?/gi,
				pReg = /<p[^>]*style\s*=\s*"[^"]*(margin-left\s*:\s*(\d+)px)[^"]*"[^>]*>/gi,//<p style="margin-left:40px;">
				pAlignReg = /<p[^>]*align\s*=\s*"?(\w+)"?[^>]*>/gi;                           //<p align="center">
			...
			data = data.replace(reg, function(match){
				...
				if((px = pReg.exec(match)) != null){
					px = parseInt(px[2]);
				}
				
				if((align = pAlignReg.exec(match)) != null){
					align = align[1];
				}
				match = match.replace(/<p[^>]*>/gi, '').replace(/\s*<blockquote[^>]*>\s*/gi, '1');
				return '<p style="margin-left:' + (40 * match.length + px) + 'px;' + (align == null ? '' : 'text-align:' + align + ';') + '">';
			});
			data = data.replace(/(<\/p>)?(\s*<\/blockquote>\s*)+/gi, '</p>');
			
			//处理列表缩进
			var listIndent = /(?:\s*<(ul|ol)[^>]*>\s*){2,}/gi;
			data = data.replace(listIndent, function(match){
				var item = 'ul',
					r = null;
				if((r = listIndent.exec(match)) != null){
					item = r[1];
				}
				
				match = match.replace(/\s*<(ul|ol)[^>]*>\s*/gi, '1');
				return '<' + item + ' style="margin-left:' + (40 * (match.length - 1)) + 'px;">';
			});
			data = data.replace(/(?:\s*<\/(ul|ol)>\s*){2,}/gi, '</$1>');
